...
class PolypPVT(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            mask = F.interpolate(mask, size=(prediction2.size(2), prediction2.size(3)), mode='nearest')
            masked_features = prediction2 * mask
            mask_areas = torch.sum(mask, dim=(2, 3))
            pooled_features = masked_features.sum(dim=[2, 3]) / mask_areas.clamp(min=1)
            
            return prediction1_8, prediction2_8, pooled_features
